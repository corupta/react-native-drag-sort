...
export default class AnySizeDragSortableView extends React.PureComponent {
  ...
  moveTouch = (nativeEvent, gestureState) => {
    ...
    if (!this.isUpdating) {
      ...
      for (let layout of layouts) {
        ...
        finalCallback = this.move(curLayout.key, layout.key, vy, curLayout.y !== layout.y)
        break
      }
      ...
      if (!this.isUpdating && nextLineLastLayout &&
        ...
        finalCallback = this.move(curLayout.key, nextLineLastLayout.key, vy, curLayout.y !== nextLineLastLayout.y)
      }
    }
  ...
  move = (fromKey, toKey, vy, isDiffline) => {
    ...
    const itemCount = this.props.countExtractor(dataSource[fromIndex]) || 1;
    ...
    if (fromIndex <= toIndex && toIndex < fromIndex + itemCount) {
      return;
    }
    this.preMoveKeyObj = {
      fromKey, toKey
    ...
    if (fromIndex < toIndex) {
      newDataSource = [
        ...dataSource.slice(0, fromIndex),
        ...dataSource.slice(fromIndex+itemCount, toIndex+1),
        ...dataSource.slice(fromIndex, fromIndex+itemCount),
        ...dataSource.slice(toIndex+1)
      ];
    } else {
      newDataSource = [
        ...dataSource.slice(0, toIndex),
        ...dataSource.slice(fromIndex, fromIndex+itemCount),
        ...dataSource.slice(toIndex, fromIndex),
        ...dataSource.slice(fromIndex+itemCount)
      ];
    }
  ...
  render() {
    ...
    const ghostKeySet = new Set();
    if (selectedKey) {
      selectedItemIndex = this.keyToIndexMap.get(selectedKey);
      selectedCount = this.props.countExtractor(selectedItem) || 1;
      for (let i = 0; i < selectedCount; ++i) {
        selectedItemList.push(dataSource[selectedItemIndex+i]);
        ghostKeySet.add(keyExtractor(dataSource[selectedItemIndex+i]))
      }
    }
    return (
      <View style={styles.box}>
        {
          selectedPosition && (
            <View style={[movedWrapStyle, styles.selectedContainer, { left: selectedPosition.left, top: selectedPosition.top, position: 'absolute', zIndex: 999 }]}>
              {selectedItemList.map((item) => (
                <View key={keyExtractor(item)}>
                  {renderItem(item, null, true)}
                </View>
              ))}
            </View>
          )
        ...
        <ScrollView
          ...
          <Transitioning.View
            ...
            transition={<Transition.Change interpolation="linear" durationMs={ANIM_DURATION} />}
          >
            {
              dataSource.map((item, index) => {
                ...
                return (
                  <View
                    key={key}
                    style={ghostKeySet.has(key) ? ghostStyle : undefined}
